...
public class GameState extends State{
	...
	public GameState()
	{
		...
		keyBoard = Window.getInstance().getKeyBoard(); // Obtenemos la instancia del KeyBoard desde Window
	}
	...
	public void update(float dt)
	{
		if(gameOver) {
			...
			// Si se presiona ENTER, volver al menú principal
			if(keyBoard.isKeyPressed(java.awt.event.KeyEvent.VK_ENTER)) {
				try {
	...
	{	
        ...
        // Dibuja los créditos si el juego ha terminado
        if(gameOver) {
            drawCredits(g2d);
        }
	}
    ...
    private void drawCredits(Graphics2D g) {
        ...
        String[] credits = {
            "SPACE SHIP GAME",
            "Score Final: " + score,
            "Oleadas completadas: " + (waves - 1),
            "Presionar Enter para continuar",
            "Creado por: Team Gato Felix",
        };

        int startY = Constants.HEIGHT / 3;
        int spacing = 50;

        // Draw title with large font
        Text.drawText(g, credits[0],
            new Vector2D(Constants.WIDTH/2, startY),
            true, Color.WHITE, Assets.fontBig);

        // Draw other credits with medium font
        for(int i = 1; i < credits.length; i++) {
            Text.drawText(g, credits[i],
                new Vector2D(Constants.WIDTH/2, startY + (i * spacing)),
	...
	public void gameOver() {
        ...
        // Añade mensaje de puntuación final
        Message finalScoreMsg = new Message(
            new Vector2D(Constants.WIDTH/2, Constants.HEIGHT/2 + 50),
            true,
        ...
        this.messages.add(finalScoreMsg);
	}
